...
fn main() {
    ...
    for _ in 0..groups {
        ...
        solver
            .add_constraint(pumpkin_solver::greater_than_or_equals(diff.clone(), 1, tag))
            .implied_by(n)
            .unwrap();
        solver
            .add_constraint(pumpkin_solver::less_than_or_equals(diff.clone(), 0, tag))
            .implied_by(!n)
            .unwrap();
        solver
            .add_constraint(pumpkin_solver::equals(vec![r.scaled(1)], 0, tag))
            .implied_by(!n)
            .unwrap();
        if variant == "full" || variant == "no-lanes" {
            ...
            solver
                .add_constraint(pumpkin_solver::greater_than_or_equals(ceiling.clone(), 0, tag))
                .implied_by(n)
                .unwrap();
            solver
                .add_constraint(pumpkin_solver::less_than_or_equals(ceiling, 1, tag))
                .implied_by(n)
                .unwrap();
        }
    }
